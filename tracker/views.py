...
def logout_page(request):
    logout(request)
    messages.error(request, "Success:Logged out successfull")
    return redirect("/login/")
...
def index(request):
    ...
    total_balance = (
        transactions.filter(created_by=request.user).aggregate(
            total_balance=Sum("amount")
        )["total_balance"]
        or 0
    )
    income = (
        transactions.filter(created_by=request.user, amount__gte=0).aggregate(
            income=Sum("amount")
        )["income"]
        or 0
    )
    expense = (
        transactions.filter(created_by=request.user, amount__lte=0).aggregate(
            expense=Sum("amount")
        )["expense"]
        or 0
    )
